...
const verperfil = async(req,res)=>{
  ...
  try {
    ...
    pool.query('select n.cliente_id, c.idempresa, c.ruc, c.email, c.nombre_empresa, c.telefono,c.direccion,c.ciudad,c.contacto,c.password,p.empresa,p.nombre_propietario,p.idusuario,p.email as corporativo,n.color,l.logo as logo_email from copia c join usuario p on c.idusuario = p.idusuario join nombres_empresa n on p.idusuario = n.idusuario join logo_empresa_email l on n.idname = l.idname where c.idempresa = $1', [userId], (err, result) => {
      if (err) {
        console.error(err);
...
const verperfilpedidos = async(req,res)=>{
  ...
  try {
    ...
    pool.query('select p.idservicio,p.cantidad,s.descripcion, c.nombre_empresa,c.telefono, s.precio,s.precio2,s.foto,p.estado,p.fecha_hora from pedido p join  servicio s on s.idservicio = p.idservicio join copia c on c.idempresa = p.idempresa where c.idempresa = $1 group by p.idservicio,p.cantidad,s.descripcion,c.nombre_empresa,c.telefono,s.precio,s.precio2,s.foto,p.estado,p.fecha_hora', [userId], (err, result) => {
      if (err) {
        console.error(err);
        res.status(500).json({ error: 'Error al obtener el perfil del usuario' });
        return;
...
const sumarpedidosclientecomprobante = async(req,res)=>{
  const token = req.headers.authorization;
...
module.exports = {
    ...
    contarcompletados,
    sumarpedidoscliente,
    sumarpedidosclientecomprobante,
    verecibosclientes,
    verecibo
}
