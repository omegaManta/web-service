...
const verperfil = async(req,res)=>{
  ...
  try {
    ...
    pool.query('select c.idempresa, c.ruc, c.email, c.nombre_empresa, c.telefono,c.direccion,c.ciudad,c.contrato,p.descripcion,p.valor from copia c join planes p on c.idplan = p.idplan where c.idempresa = $1', [userId], (err, result) => {
      if (err) {
        console.error(err);
...
const verperfilpedidos = async(req,res)=>{
  ...
  try {
    ...
    pool.query('select p.idpedido,s.descripcion, c.nombre_empresa,c.telefono,c.contrato, s.precio, s.duracion,i.img,p.estado,p.fecha_hora from pedido p join  servicio s on s.idservicio = p.idservicio join copia c on c.idempresa = p.idempresa join imgservicio i on s.idservicio,i.idservicio where c.idempresa = $1 group by p.idpedido,s.descripcion,c.nombre_empresa,c.telefono,c.contrato,s.precio,s.duracion,i.img,p.estado,p.fecha_hora', [userId], (err, result) => {
      if (err) {
        console.error(err);
        res.status(500).json({ error: 'Error al obtener el perfil del usuario' });
        return;
