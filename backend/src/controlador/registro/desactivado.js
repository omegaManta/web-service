...
const creardesactivado = async (req, res) => {
  ...
  const response = await pool.query('select d.idEmpresa, d.ruc, d.email, d.telefono, d.direccion, d.nombre_empresa, d.contacto, d.ciudad, d.password, d.idusuario,d.fecha_ingreso,d.contrato, p.nombres_empresa from usuario p join desactivado d on p.idusuario = d.idusuario')
  ...
  const buscardesactivados = async(req,res)=>{
    ...
    const response = await pool.query('select d.idEmpresa, d.ruc, d.email, d.telefono, d.direccion, d.nombre_empresa, d.contacto, d.ciudad, d.password, d.idusuario,d.fecha_ingreso, p.nombres_empresa,d.contrato from usuario p join desactivado d on p.idusuario = d.idusuario where d.nombre_empresa like $1',[
      nombre_empresa + '%'
    ])
    res.status(200).json(response.rows)
  }
